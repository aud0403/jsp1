...
public class MybatisDao {
		...
		public List<Customer> selectAll(){
			...
			list= mapper.selectList("selectAll");
							//ㄴ 이 메소드는 리턴타입이 List<Object> select 조회결과가 n개 row 일때
							//ㄴ customer.xml에서 태그의 id
			mapper.close();
			return list;
		}
		...
		public Customer select(int idx) {
			Customer cus =null;
			SqlSession mapper = sqlFactory.openSession();
			cus = mapper.selectOne("seletOne",idx);  //두번째 인자는 sql실행에 필요한 파라미터입니다.
							//ㄴ 이 메소드는 리턴타입이 Object, select 조회결과가 1row 일때
			mapper.close();
			return cus;
		}
		...
		public int insert(Customer cus) {
			...
			mapper.insert("insert",cus);		//auto commit? n 값도 확인  정상 insert이면 1
